...
export const DeskUtilities: React.FC = () => {
  ...
  const [foundOnlineBook, setFoundOnlineBook] = useState<{
    isbn: string;
    title: string;
    author: string;
    coverUrl?: string;
  ...
  // Process barcode input (from physical gun, camera feed, or manual typing)
  const processBarcodeInput = async (barcodeText: string) => {
    const cleanInput = barcodeText.trim();
    ...
    // 2. Try to match an existing Book in catalog by barcode (ISBN, ID, deweyCode)
    const matchedBook = books.find(b => 
      b.isbn.toLowerCase() === cleanInput.toLowerCase() || 
      b.id.toLowerCase() === cleanInput.toLowerCase() ||
      b.isbn.replace(/[^0-9X]/gi, '').toLowerCase() === cleanInput.replace(/[^0-9X]/gi, '').toLowerCase()
    );
          ...
          if (loadedUser) {
            ...
            if (activeLoan) {
              ...
              if (res.success) {
                setScannerMessage({
                  type: 'success',
                  text: `Scan Verified! Automatically RENEWED loan of "${matchedBook.title}" for ${loadedUser.name} for another ${days} days.`
                });
              } else {
    ...
    // 3. Fallback search by Name or Title
    ...
    const matchedBookByTitle = books.find(b => 
      b.title.toLowerCase().includes(cleanInput.toLowerCase()) || 
      b.author.toLowerCase().includes(cleanInput.toLowerCase())
    );
    ...
    // 4. If looks like a valid 10- or 13-digit ISBN not yet accessioned in catalog, query Open Library!
    const digitsOnly = cleanInput.replace(/[^0-9X]/gi, '');
    if (digitsOnly.length === 10 || digitsOnly.length === 13) {
      setIsLookingUpISBN(true);
      setScannerMessage({
        type: 'success',
        text: `ISBN scanned: ${cleanInput}. Querying Open Library global database for title metadata...`
      });
      try {
        const lookup = await lookupBookByISBN(digitsOnly);
        if (lookup.success && lookup.book) {
          setFoundOnlineBook({
            isbn: cleanInput,
            title: lookup.book.title,
            author: lookup.book.authors.join(', '),
            coverUrl: lookup.book.coverUrl,
            description: lookup.book.description,
            subjects: lookup.book.subjects
          });
          setScannerMessage({
            type: 'success',
            text: `Found "${lookup.book.title}" by ${lookup.book.authors.join(', ')} via Open Library! Click "Accession to Catalog" below to add copies to your library.`
          });
          ...
          setScannerMessage({
            type: 'error',
            text: `Scanned ISBN "${cleanInput}" is not in local catalog, and Open Library returned: ${lookup.error || 'Title not indexed yet'}.`
          });
        }
      } catch (err: any) {
        setScannerMessage({
          type: 'error',
          text: `Could not query online ISBN catalog: ${err.message}`
        });
      } finally {
        setIsLookingUpISBN(false);
      }
      ...
      setScannerMessage({
        type: 'error',
        text: `No match found for input: "${cleanInput}". Scan a valid user card ID (e.g. LIB-xxxx) or book ISBN.`
      });
    }
    setPhysicalScanText('');
  };
  ...
  const handlePhysicalBarcodeSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    processBarcodeInput(physicalScanText);
  };
  ...
  return (
    <div className="space-y-8 print:bg-white print:p-0">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-black text-cyan-400 uppercase tracking-widest">
                  HARDWARE & OPTICAL BARCODE SCANNER
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  ...
                  <span>{showCameraScanner ? 'Close Live Camera' : 'Turn On Live Camera'}</span>
                </button>
                <span className="text-[10px] text-slate-400 font-mono italic hidden md:inline">
                  USB/Bluetooth gun or camera
                </span>
              </div>
            </div>

            {/* Live Camera Scanner Viewport when toggled */}
            <AnimatePresence>
              {showCameraScanner && (
                <motion.div
                ...
                >
                  <CameraBarcodeScanner
                    onScan={(decoded) => {
                      processBarcodeInput(decoded);
                    }}
                    onClose={() => setShowCameraScanner(false)}
                    title="Desk Optical Barcode & ISBN Scanner"
                    subtitle="Point your camera at a book ISBN barcode or student library badge"
                  />
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <form onSubmit={handlePhysicalBarcodeSubmit} className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  type="text"
                  autoFocus
                  placeholder="Scan physical Library Card (LIB-xxxx), Book ISBN, or type to search..."
              ...
              <button
                ...
                className="bg-gradient-to-r from-cyan-500 to-indigo-600 hover:from-cyan-400 hover:to-indigo-500 disabled:opacity-50 text-slate-950 hover:text-white font-mono font-black text-xs px-8 py-3.5 rounded-2xl cursor-pointer shadow-lg transition-all flex items-center justify-center gap-2"
              >
                {isLookingUpISBN && <RefreshCw className="w-3.5 h-3.5 animate-spin" />}
                <span>{isLookingUpISBN ? 'QUERYING ISBN...' : 'PARSE BARCODE'}</span>
              </button>
            </form>
          </div>
              ...
              <div className="bg-slate-950 border border-slate-800 rounded-3xl p-5 shadow-inner space-y-4 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    <span>{showCameraScanner ? 'Hide Camera' : 'Use Camera'}</span>
                  </button>
                </div>
                  ...
                  <AnimatePresence>
                    {isLookingUpISBN ? (
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0 }}
                        className="text-center z-10 text-amber-400 font-mono text-xs space-y-2"
                      >
                        <RefreshCw className="w-8 h-8 animate-spin mx-auto text-amber-400" />
                        <p className="animate-pulse">FETCHING ISBN FROM OPEN LIBRARY...</p>
                      </motion.div>
                    ) : scanAnimation ? (
                      <motion.div 
                        initial={{ opacity: 0, scale: 0.8 }}
      ...
      {/* Modal for Accessioning an ISBN scanned from Open Library */}
      <AnimatePresence>
        {foundOnlineBook && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
            ...
            >
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h3 className="font-display font-black text-base text-slate-900">
                      New Book Discovered via ISBN Scan
                    </h3>
                    <p className="text-[11px] text-slate-500">
                      ISBN: <span className="font-mono font-semibold">{foundOnlineBook.isbn}</span> · Open Library Metadata
                    </p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setFoundOnlineBook(null)}
              ...
              <div className="flex gap-4 items-start">
                {foundOnlineBook.coverUrl ? (
                  <img
                    src={foundOnlineBook.coverUrl}
                    alt={foundOnlineBook.title}
                    referrerPolicy="no-referrer"
                    className="w-20 h-28 object-cover rounded-xl shadow-md border border-slate-200 shrink-0"
                  />
                ) : (
                  <div className="w-20 h-28 bg-slate-100 rounded-xl border border-slate-200 flex flex-col items-center justify-center text-slate-400 shrink-0">
                    <BookOpen className="w-6 h-6" />
                    <span className="text-[9px] mt-1 font-mono">No Cover</span>
                  </div>
                )}

                <div className="space-y-1.5 flex-1 min-w-0">
                  <h4 className="font-display font-extrabold text-sm sm:text-base text-slate-900 leading-tight">
                    {foundOnlineBook.title}
                  </h4>
                  <p className="text-xs text-slate-600 font-medium">
                    By {foundOnlineBook.author || 'Unknown Author'}
                  </p>
                  {foundOnlineBook.description && (
                    <p className="text-[11px] text-slate-500 line-clamp-3 leading-relaxed">
                      {foundOnlineBook.description}
                    </p>
                  )}
                  {foundOnlineBook.subjects && foundOnlineBook.subjects.length > 0 && (
                    <div className="flex flex-wrap gap-1 pt-1">
                      {foundOnlineBook.subjects.slice(0, 3).map((s) => (
                        <span
                          key={s}
              ...
              <div className="bg-emerald-50 border border-emerald-200/80 rounded-2xl p-3 text-xs text-emerald-950 flex items-center gap-2">
                <Check className="w-4 h-4 text-emerald-600 shrink-0" />
                <span>
                  Would you like to accession this physical copy directly into your library catalog and Supabase database?
                </span>
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const deweyMap: Record<string, string> = {
                      'Fiction': '800',
                      'African': '896.3',
                      'Science': '500',
                      'Technology': '600',
                    ...
                    const primarySubject = foundOnlineBook.subjects?.[0] || 'General';
                    const category = primarySubject.includes('Africa') 
                      ? 'African Literature' 
                      : primarySubject.includes('Sci') 
                      ? 'STEM & Space' 
                      : primarySubject.includes('Code') || primarySubject.includes('Comput')
                      ? 'Coding & Tech'
                      : 'Classics';

                    addBook({
                      title: foundOnlineBook.title,
                      author: foundOnlineBook.author || 'Unknown',
                      isbn: foundOnlineBook.isbn,
                      category,
                      totalCopies: 3,
                      availableCopies: 3,
                      description: foundOnlineBook.description || `Catalog entry accessioned via ISBN scan: ${foundOnlineBook.isbn}`,
                      coverImage: foundOnlineBook.coverUrl || 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&q=80&w=700',
                      coverUrl: foundOnlineBook.coverUrl || 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&q=80&w=700',
                      deweyClass: deweyMap[primarySubject] || '800',
                      deweyCode: '823.9',
                      callNumber: `823.9 ${foundOnlineBook.author.substring(0, 3).toUpperCase()}`,
                      ageRange: 'Ages 10-18',
                      readingLevel: 'Lexile 850L',
                      pageCount: 280,
                      hasAudio: false,
                      isAudiobook: false,
                      isPopular: true,
                    ...
                    setScannerMessage({
                      type: 'success',
                      text: `Successfully accessioned "${foundOnlineBook.title}" (ISBN: ${foundOnlineBook.isbn}) into library catalog!`
                    });
                    setFoundOnlineBook(null);
                  }}
                  className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold rounded-xl shadow-xs cursor-pointer transition active:scale-95 flex items-center gap-1.5"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>Accession 3 Copies to Catalog</span>
                </button>
              </div>
            </motion.div>
          </div>
